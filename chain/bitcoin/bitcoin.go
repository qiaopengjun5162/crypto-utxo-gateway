...
import (
	...
	"github.com/qiaopengjun5162/crypto-utxo-gateway/chain/base"
	"github.com/qiaopengjun5162/crypto-utxo-gateway/chain/bitcoin/types"
	"github.com/qiaopengjun5162/crypto-utxo-gateway/config"
	common2 "github.com/qiaopengjun5162/crypto-utxo-gateway/rpc/common"
...
type ChainAdaptor struct {
	btcClient       *base.Client
	btcDataClient   *base.DataClient
...
// GetSupportChains implements the IChainAdaptor interface.
// It checks if the chain is supported by this ChainAdaptor.
func (c *ChainAdaptor) GetSupportChains(req *utxo.SupportChainsRequest) (*utxo.SupportChainsResponse, error) {
	// Currently, we only support Bitcoin.
	// If the chain name is not "Bitcoin", we return an error.
	if req.Chain != ChainName {
		return nil, errors.New("this chain is not supported")
	}
...
// It converts a public key to a specified format of address.
// The formats supported are "p2pkh", "p2wpkh", "p2sh", and "p2tr".
// If the format is not specified, it will return an error.
// The response will contain the converted address string.
...
// ValidAddress implements the IChainAdaptor interface.
// It verifies if the given address is valid or not.
// The response will contain the result of verification.
// If the address is invalid, the response will contain an error message.
func (c *ChainAdaptor) ValidAddress(req *utxo.ValidAddressRequest) (*utxo.ValidAddressResponse, error) {
	// Decode the address using the Bitcoin mainnet parameters.
	...
	// Check if the address is valid for the Bitcoin mainnet.
	if !address.IsForNet(&chaincfg.MainNetParams) {
		// Return an error response if the address is not valid for this network.
		return &utxo.ValidAddressResponse{
			Code: common2.ReturnCode_ERROR,
	...
	// Return a successful response indicating the address is valid.
	return &utxo.ValidAddressResponse{
		Code:  common2.ReturnCode_SUCCESS,
...
// GetBlockHeaderByHash implements the IChainAdaptor interface.
// It gets the block header with the given block hash.
// The response will contain the block header data.
// If the block hash is invalid, or the block header can not be found,
// it will return an error code and message.
//
// Parameters:
//   - req: A BlockHeaderHashRequest containing the block hash.
//
// Returns:
//   - *utxo.BlockHeaderResponse: Contains the block header data if successful,
//     or an error message if not.
//   - error: An error object if the block header can not be found.
func (c *ChainAdaptor) GetBlockHeaderByHash(req *utxo.BlockHeaderHashRequest) (*utxo.BlockHeaderResponse, error) {
	if req == nil || req.Hash == "" {
		return &utxo.BlockHeaderResponse{
			Code: common2.ReturnCode_ERROR,
			Msg:  "invalid request: empty hash",
		}, fmt.Errorf("invalid request: empty hash")
	}

	hash, err := chainhash.NewHashFromStr(req.Hash)

	if err != nil {
		log.Error("format string to hash fail", "hash", req.Hash, "err", err)
		return &utxo.BlockHeaderResponse{
			Code: common2.ReturnCode_ERROR,
			Msg:  "invalid block hash format",
		}, fmt.Errorf("invalid block hash format: %v", err)
	}
	blockHeader, err := c.btcClient.Client.GetBlockHeader(hash)
	if err != nil {
		log.Error("get block header fail", "hash", req.Hash, "err", err)
		return &utxo.BlockHeaderResponse{
			Code: common2.ReturnCode_ERROR,
			Msg:  "failed to get block header",
		}, fmt.Errorf("failed to get block header: %v", err)
	}
...
// GetBlockHeaderByNumber implements the IChainAdaptor interface.
// It gets the block header with the given block number.
func (c *ChainAdaptor) GetBlockHeaderByNumber(req *utxo.BlockHeaderNumberRequest) (*utxo.BlockHeaderResponse, error) {
	// If the height is 0, get the latest block number.
	...
	// Get the block hash with the given block number.
	...
	// Get the block header with the given block hash.
...
// SendTx broadcasts a raw transaction to the Bitcoin network.
// It takes a SendTxRequest containing the raw transaction as input,
// deserializes it, and sends it using the btcClient. If successful,
// it returns a SendTxResponse with the transaction hash and a success code.
// Otherwise, it returns an error code and message.
//
// Params:
//   - req: A SendTxRequest containing the raw transaction data.
//
// Returns:
//   - *utxo.SendTxResponse: Contains the transaction hash if successful,
//     or an error message if not.
//   - error: An error object if the transaction could not be sent.
